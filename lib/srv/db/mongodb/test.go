...
func (s *TestServer) handleConnection(conn net.Conn) error {
	// Handle authentication - Teleport will send auth message first
	// thing after connecting.
	message, err := protocol.ReadMessage(conn)
	if err != nil {
		return trace.Wrap(err)
	}
	reply, err := s.handleAuth(message)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	// basic set of commands such as "isMaster", "ping" and "find".
	for {
		message, err := protocol.ReadMessage(conn)
		if err != nil {
			return trace.Wrap(err)
...
func (s *TestServer) handleAuth(message protocol.Message) (protocol.Message, error) {
	authCmd, ok := message.(*protocol.MessageOpMsg)
	if !ok {
		return nil, trace.BadParameter("expected OP_MSG message, got: %v", message)
	}
	document, err := authCmd.GetDocument()
	if err != nil {
		return nil, trace.Wrap(err)
	}
	if getCommandType(document) != commandAuth {
		return nil, trace.BadParameter("expected authenticate command, got: %v", document)
	}
