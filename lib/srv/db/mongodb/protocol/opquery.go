...
func (m *MessageOpQuery) String() string {
	return fmt.Sprintf("OpQuery(FullCollectionName=%v, Query=%v)",
		m.FullCollectionName, m.Query.String())
}
...
func (m *MessageOpQuery) ToWire(responseTo int32) (dst []byte) {
	var idx int32
	idx, dst = wiremessage.AppendHeaderStart(dst, 0, responseTo, wiremessage.OpQuery)
