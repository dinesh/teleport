...
func (m *MessageOpMsg) GetDatabase() string {
	for _, document := range m.GetDocuments() {
		if value := document.Lookup("$db"); value.StringValue() != "" {
...
func (m *MessageOpMsg) String() string {
	return fmt.Sprintf("OpMsg(Documents=%v)", m.GetDocumentsAsStrings())
}
...
func (m *MessageOpMsg) ToWire(responseTo int32) (dst []byte) {
	var idx int32
	idx, dst = wiremessage.AppendHeaderStart(dst, int32(rand.Intn(10000)), responseTo, wiremessage.OpMsg)
