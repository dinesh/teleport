...
<Admonition type="warning" title="Warning">
  Backing up production instances and/or configurations before making changes or modifications is a best practice. We encourage this so you can roll back to an existing state as needed.
</Admonition>
...
The configuration above is all you need to `ssh root@db` if an SSH agent is running on a client computer. You can verify it by executing `ssh-add -L` right after `tsh login`. If the SSH agent is running, the cluster certificates will
...
eval `ssh-agent`
# Agent pid 8403
tsh login --proxy=root.example.com --user=teleport-admin
```
...
## Quick connect using OpenSSH

This section describes how to connect to a Teleport Auth node in quickstart format.

<Admonition type="warning" title="Warning">
  Backing up production instances and/or configurations before making changes or modifications is a best practice. We encourage this so you can roll back to an existing state as needed.
</Admonition>

1. On your client machine, you need to import the public key of Teleport's host certificate. This will allow your OpenSSH client to verify that host certificates are signed by Teleport's trusted host Certificate Authority (CA). On your Teleport Auth server run:
...
2. Verify that you have `ssh-agent` running on your local client machine:
   ...
   Teleport `tsh` will save SSH information to `ssh-agent` by default.

3. Log in to the Teleport Auth server with `tsh`.

   ```bash
   tsh login --proxy=root.example.com --user=teleport-admin
   ```

4. Modify your `ssh` configuration files to use a Teleport proxy on `proxy.example.com`. 

   These are found at `/etc/ssh/ssh_config` or `~/.ssh/config`. 
...
5. Use OpenSSH to connect:

   ```bash
   ssh root@openssh-test2.centralus.cloudapp.azure.com -p 3022
   ```