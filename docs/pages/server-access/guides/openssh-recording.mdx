...
Unless you've highly automated your infrastructure, we would suggest you proceed with caution as this will invalidate the user and host Certificate Authority's, meaning that the new Certificate Authority's will need to be exported to every OpenSSH-based machine again using `tctl auth export` as above.